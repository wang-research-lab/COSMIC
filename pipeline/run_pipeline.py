...
from pipeline.utils.orig_hook_utils import get_activation_addition_input_pre_hook, get_all_direction_ablation_hooks
...
def generate_and_save_completions_for_dataset(cfg, model_base, fwd_pre_hooks, fwd_hooks, intervention_label, dataset_name, dataset=None, harmtype = None):
    ...
    if dataset is None:
        random.seed(42)
    if harmtype:
        dataset = random.sample(load_dataset_split(harmtype=harmtype, split='test', instructions_only=True), cfg.n_test)

    completions = model_base.generate_completions(dataset, fwd_pre_hooks=fwd_pre_hooks, fwd_hooks=fwd_hooks, max_new_tokens=cfg.max_new_tokens, batch_size = 128)
...
def run_pipeline(model_path):
    ...
    model_alias = os.path.basename(model_path) + "-rdt"
    ...
    actadd_fwd_pre_hooks, actadd_fwd_hooks = [(model_base.model_block_modules[layer], get_activation_addition_input_pre_hook(vector=direction, coeff=torch.Tensor([-1.0])))], []
    ...
    actadd_refusal_pre_hooks, actadd_refusal_hooks = [(model_base.model_block_modules[layer], get_activation_addition_input_pre_hook(vector=direction, coeff=torch.Tensor([1.0])))], []
