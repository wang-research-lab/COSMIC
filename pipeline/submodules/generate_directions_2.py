...
# def get_mean_activations_post_attn(layer, cache: Float[Tensor, "pos layer d_model"], n_samples, positions: List[int]):
#     def hook_fn(module, input, output):
#         activation: Float[Tensor, "batch_size seq_len d_model"] = input[0].clone().to(cache)
#         cache[:, layer] += (1.0 / n_samples) * activation[:, positions, :].sum(dim=0)
#     return hook_fn
...
def get_mean_activations(model, tokenizer, instructions, tokenize_instructions_fn, block_modules: List[List[torch.nn.Module]], batch_size=32, positions=[-1]):
    ...
    fwd_pre_hooks = [(block_modules[layer], get_mean_activations_pre_hook(layer=layer, cache=pre_mean_activations, n_samples=n_samples, positions=positions)) for layer in range(n_layers)]
    fwd_pre_hooks += [(block_modules[layer].post_attention_layernorm, get_mean_activations_pre_hook(layer=layer, cache=post_attn_mean_activations, n_samples=n_samples, positions=positions)) for layer in range(n_layers)]
    fwd_hooks = [(block_modules[layer], get_mean_activations_post_hook(layer=layer, cache=post_mlp_mean_activations, n_samples=n_samples, positions=positions)) for layer in range(n_layers)]
    ...
    return torch.stack((pre_mean_activations, post_attn_mean_activations, post_mlp_mean_activations), dim=0)
