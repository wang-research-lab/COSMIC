...
from pipeline.utils.hook_utils import get_activation_addition_input_pre_hook, get_all_direction_ablation_hooks, get_direction_ablation_input_pre_hook, get_activation_addition_input_pre_hook

from pipeline.submodules.generate_directions_cosmic import generate_directions
from pipeline.submodules.select_direction_ace_substring import select_direction, get_refusal_scores
...
def run_pipeline(model_path):
    ...
    model_alias = os.path.basename(model_path) + "-acerdt"
    cfg = Config(model_alias=model_alias, model_path=model_path)
    print(cfg.model_alias)
    ...
    pos, layer, direction, harmless_reference, ablation_enhance_coeff  = select_and_save_direction(cfg, model_base, harmful_val, harmless_val, candidate_directions, harmless_mean)
    ...
    ablation_fwd_pre_hooks = [(model_base.model_block_modules[layer], get_direction_ablation_input_pre_hook(direction=post_layer_direction, coeff = coeff, reference = post_layer_reference))]
    ...
    actadd_fwd_pre_hooks = [(model_base.model_block_modules[layer], get_activation_addition_input_pre_hook(direction=post_layer_direction, coeff = coeff, reference = post_layer_reference))]
