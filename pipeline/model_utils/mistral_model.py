...
# Mistral chat templates are similar to LLaMA but adapted for its format
MISTRAL_CHAT_TEMPLATE = """[INST] {instruction} [/INST] """
MISTRAL_CHAT_TEMPLATE_WITH_SYSTEM = """<s>[SYSTEM] {system_prompt} [/SYSTEM] [INST] {instruction} [/INST] """

MISTRAL_REFUSAL_TOKS = [40]  # Example token ID, adjust as needed
...
def format_instruction_mistral_chat(
    ...
    if system is not None:
        formatted_instruction = MISTRAL_CHAT_TEMPLATE_WITH_SYSTEM.format(instruction=instruction, system_prompt=system)
    else:
...
def tokenize_instructions_mistral_chat(
    ...
    if outputs is not None:
        prompts = [
            format_instruction_mistral_chat(instruction=instruction, output=output, system=system, include_trailing_whitespace=include_trailing_whitespace)
            for instruction, output in zip(instructions, outputs)
        ]
    else:
        prompts = [
            format_instruction_mistral_chat(instruction=instruction, system=system, include_trailing_whitespace=include_trailing_whitespace)
...
class MistralModel(ModelBase):
    ...
    def _get_eoi_toks(self):
        return self.tokenizer.encode(MISTRAL_CHAT_TEMPLATE.split("{instruction}")[-1], add_special_tokens=False)
